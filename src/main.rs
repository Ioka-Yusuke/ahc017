    ...
    for i in 0..m {
        ...
        for u_i in &stop_days_u {
            for v_i in &stop_days_v {
                if u_i == v_i {
                    let sum_w: usize = root_weight[*(u_i)][(u-1) as usize]+root_weight[*(v_i)][(v-1) as usize];
                    rw.push((u_i, sum_w));
                }
            }
        }
        ...
        if rw.len() != 0 && now_ans[i] == 1_000_000_000 {
            ...
            pre_conect_roots.push((u, v, i));
        }
    }

    // 3頂点最短
    for i in 0..pre_conect_roots.len() {
        let num: usize = pre_conect_roots[i].2;
        let u: usize = pre_conect_roots[i].0;
        let v: usize = pre_conect_roots[i].1;
        ...
        for u_i in &stop_days_u {
            for v_i in &stop_days_v {
                if root_weight[*(u_i)][*(v_i)] != 1_000_000_000 {
                    let sum_w: usize = root_weight[*(v_i)][(u-1) as usize]+root_weight[*(u_i)][(v-1) as usize]+root_weight[*(u_i)][*(v_i)];
                    rw.push((sum_w, u_i, v_i));
                }
            }
        }

        if rw.len() != 0 && now_ans[num] == 1_000_000_000 {
            rw.sort();
            let sum_w_u_i_v_i = rw[0];
            let u_i = sum_w_u_i_v_i.1;
            let v_i = sum_w_u_i_v_i.2;
            ...
            if now_ans[(root_m[*(u_i)][(u-1) as usize]) as usize] != 1_000_000_000 {
                random_seed.retain(|&x| x != now_ans[(root_m[*(u_i)][(u-1) as usize]) as usize]);
            }

            if now_ans[(root_m[*(v_i)][(v-1) as usize]) as usize] != 1_000_000_000 {
                random_seed.retain(|&x| x != now_ans[(root_m[*(v_i)][(v-1) as usize]) as usize]);
            }

            if now_ans[(root_m[*(v_i)][*(u_i)]) as usize] != 1_000_000_000 {
                random_seed.retain(|&x| x != now_ans[(root_m[*(v_i)][*(u_i)]) as usize]);
            }
            ...
            daily_uvw_map[(day) as usize][(u-1) as usize][(v-1) as usize] = STOP_VALUE;
            daily_uvw_map[(day) as usize][(v-1) as usize][(u-1) as usize] = STOP_VALUE;
            short_roots[*(u_i)][(u-1) as usize].push(day);
            short_roots[*(v_i)][(v-1) as usize].push(day);
            short_roots[(u-1) as usize][*(u_i)].push(day);
            short_roots[(v-1) as usize][*(v_i)].push(day);
            short_roots[*(u_i)][*(v_i)].push(day);
            short_roots[*(v_i)][*(u_i)].push(day);

            now_ans[num] = (day) as usize;

        } else if now_ans[num] == 1_000_000_000 {
