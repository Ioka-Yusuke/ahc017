...
fn main() {
    ...
    // 全体座標の平均
    let mut avg_all_x = 0.0;
    let mut avg_all_y = 0.0;
    for i in 0..n {
        avg_all_x += xy[i][0];
        avg_all_y += xy[i][1];
    }
    ...
    loop {
        ...
        let mut now_sum_abs = 0.0;
        let mut fut_sum_abs = 0.0;
        let mut cnt_k = 0;
        ...
        for day in [now_day, fut_day] {
            ...
            now_sum_abs += (avg_now_x - avg_all_x).abs() + (avg_now_y - avg_all_y).abs(); 
            ...
            fut_sum_abs += (avg_fut_x - avg_all_x).abs() + (avg_fut_y - avg_all_y).abs(); 
        }
        
        
        

        if fut_sum_covariance > now_sum_covariance && fut_sum_abs < now_sum_abs && cnt_k <= k {
